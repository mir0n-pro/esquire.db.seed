...
BEGIN
    ...
    FOR o IN (WITH RECURSIVE org_tree AS (
              SELECT  org_et_pk, org_pk, org_org_pk, 1 AS level, org_name, org_desc
                FROM esq_org WHERE org_pk = 1
              UNION ALL 
              SELECT child.org_et_pk, child.org_pk, child.org_org_pk, parent.level + 1, child.org_name, child.org_desc
                FROM esq_org child
