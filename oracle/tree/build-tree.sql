...
BEGIN
    -- build org tree
    FOR o IN (SELECT org_et_pk, org_pk, org_org_pk, LEVEL, org_name, org_desc, org_path
    FROM esq_org
 ...
 --             FROM esq_tree START WITH tree_tree_pk_parent IS NULL CONNECT BY PRIOR  tree_pk = tree_tree_pk_parent ) LOOP
    FOR a IN (SELECT tree_pk, LEVEL, SUBSTR(SYS_CONNECT_BY_PATH( tree_pk, '.'),2) ||'.'  AS path 
              FROM esq_tree START WITH tree_tree_pk_parent IS NULL CONNECT BY PRIOR  tree_pk = tree_tree_pk_parent ) LOOP
...
SELECT tree_pk, CAST(SUBSTR(RPAD(' ', LEVEL, '|')||'-'||tree_name, 1,50) AS VARCHAR2(50)) name, LEVEL, TREE_LEVEL, CAST(tree_path AS VARCHAR2(50)) path, CAST(tree_entity_path AS VARCHAR2(50)) epath 
FROM esq_tree
